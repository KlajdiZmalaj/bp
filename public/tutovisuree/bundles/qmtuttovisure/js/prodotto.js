$(document).ready(function () {
    ...
    function updateOraUrgenza() {
        $('.oraUrgenza').html(moment().add(Tuttovisure.parziali.tempiUrgenza, 'minute').format('H:mm'));
    }
    ...
    function stickyTotal() {
        if(window.innerWidth <= 990 && idPage.className !== "pagamenti") {
            ...
            var stop = divScroll.offsetTop + formHeight + descrProdHeight + faqHeight + 101;
            var contWidth = $('#stickyHeight').width();

            var riepilogoHeight = $("#riepilogoSticky #riepilogo").height()
            if (riepilogoHeight === 0) {riepilogoHeight = 69;}
            if (window.pageYOffset >= start && window.pageYOffset <= stop - riepilogoHeight - 50) {
                $('#riepilogoSticky div:first-child').addClass("fixed");
                $('#riepilogoSticky').addClass("fixed");
                $('#mobileSubmit').css("top", riepilogoHeight + 17);
                $('#btnUp').css("right", (window.innerWidth - 1140) / 2).show();
                $('#tempi-sticky').addClass("fixed").css({"top": riepilogoHeight + 45, "right": (window.innerWidth - contWidth + 2) / 2});
            } else {
    ...
    $("label.urgenza-riepilogo,.fakeInput,#urg-riepilogo").mousedown(function(event) {
        event.preventDefault();
    });


    $("#prodotto-form .widget > div").has("input[type=\"radio\"]").parent().closest('div.row').addClass('radiob');
