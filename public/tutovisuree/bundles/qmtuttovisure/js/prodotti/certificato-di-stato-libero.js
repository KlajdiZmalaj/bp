...
$(window).on('load', function() {
    if ($("#certificato_di_stato_libero_ordine_cliente_tipoCliente").length || $("#certificato_di_stato_libero_ordine_tipoCheckOut").length) {
        setTimeout (function () {
...
$(document).ready(function(){
    ...
    $("#certificato_di_stato_libero_ordine_opzioniProdotto_certificatoDaTradurreELegalizzare").logicaOpzioniProdotto( {
        'prefix_form': 'certificato_di_stato_libero_ordine_',
        'valori': {
            '1': {
                'mostra': [
                    'lingue',
    ...
    $("#certificato_di_stato_libero_ordine_opzioniProdotto_certificatoDaTradurreELegalizzare").change(function() {
        if(!$(this).is(':checked')) {
    ...
    $("#certificato_di_stato_libero_ordine_opzioniProdotto_certificatoDaTradurreELegalizzare").logicaOpzioniProdotto().refreshOpzioni();

    $("#certificato_di_stato_libero_ordine_datiProdotto_provincia").change(function() {
        Tuttovisure.provinciaComune($(this), "#certificato_di_stato_libero_ordine_datiProdotto_comune");
    });

    $("#certificato_di_stato_libero_ordine_datiProdotto_traduzione").change(function() {
        Tuttovisure.Ordine.aggiornaParziali();
    });

    $("input[name$='certificato_di_stato_libero_ordine[spedizione][spedizione_nazionale]']").change(function () {
    ...
    if ($("#certificato_di_stato_libero_ordine_spedizione_spedizione_nazionale_3").is(':checked')) {
        $('.spedizione_internazionale').removeClass('hidden');
        $('.spedizione_internazionale').children('.form-control').removeClass('hidden');
        $('.spedizione_internazionale').find('input:radio').prop('required', true);
    } else {
